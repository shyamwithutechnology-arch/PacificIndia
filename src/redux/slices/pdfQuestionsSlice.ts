...
const pdfQuestionsSlice = createSlice({
  ...
  reducers: {
    ...
    addChapterQuestions: (
      ...
      const existingChapterIndex = state.chapters.findIndex(
        (chap) =>
          chap.chapterId === chapterId && chap.questionTypeId === questionTypeId
      );
      ...
      if (existingChapterIndex >= 0) {
        ...
        const existingQuestionNumbers = new Set(
          existingChapter.questionNumbers
        );
      ...
      const existingQuestionIds = new Set(
        state.allQuestions.map((q) => q.question_id)
      );
    ...
    removeChapterQuestions: (
      ...
      if (chapterIndex >= 0) {
        ...
        state.chapters[chapterIndex].selectedQuestions = state.chapters[
          chapterIndex
        ].selectedQuestions.filter((q) => !questionIds.includes(q.question_id));

        // Update question numbers
        state.chapters[chapterIndex].questionNumbers = state.chapters[
          chapterIndex
        ].questionNumbers.filter((num) => {
    ...
    addPDFQuestions: (state, action: PayloadAction<SelectedQuestion[]>) => {
      const newQuestions = action.payload.filter(
        (newQ) =>
          !state.allQuestions.some(
            (existing) => existing.question_id === newQ.question_id
          )
      );
      state.allQuestions.push(...newQuestions);
    },
    ...
    removePDFQuestions: (state, action: PayloadAction<string[]>) => {
      ...
      state.chapters = state.chapters
        .map((chapter) => {
          ...
          const filteredQuestionNumbers = chapter.questionNumbers.filter(
            (num, index) => {
